...
namespace Ashsvp
{
    public class V2_Online_SimcadeVehicleController : NetworkBehaviour
    {
        ...
        [Header("Other Things")] private V2_Online_VehiclesInputs _input;
        ...
        private void Update()
        {
            ...
            {
                accelerationInput = _input._accelerationValue;
                steerInput = _input._steerValue;
                brakeInput = _input._brakeValue;
            }
            ...
            CmdSendInput(accelerationInput, steerInput, brakeInput);
        }

        [Command]
        private void CmdSendInput(float acceleration, float steer, float brake)
        {
            ...
            float factor)
        {
            if (wheelIsGrounded)
            {
                ...
                Vector3 frictionForce = Vector3.ClampMagnitude(rb.mass / 4 * contactDesiredAccel,
                    springForce * FrictionCoefficient);
            ...
            float factor, float suspensionForce, int wheelNum)
        {
            if (wheelIsGrounded)
            {
                ...
                Vector3 frictionForce = Vector3.ClampMagnitude(
                    contactDesiredAccel * (rb.mass * sideFrictionCurve.Evaluate(slipCoeff[wheelNum])),
                    suspensionForce * FrictionCoefficient);
                frictionForce = -sideVelocity.normalized * (suspensionForce * FrictionCoefficient *
